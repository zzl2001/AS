        ...
        switch (view.getId()){
            ...
            case  R.id.btn_openSchool:
                    if (b_accessblity) {
                        ...
                        Toast.makeText(getApplicationContext(), "未打开无障碍服务", Toast.LENGTH_SHORT).show();
                        }
                break;
            case R.id.btn_shutDownService:
                try {
                    MyApplication.getInstance().setFlag(false);
                    Toast.makeText( getApplicationContext(),"关闭脚本成功", Toast.LENGTH_SHORT).show();
                }catch (Exception e){
                    Toast.makeText(getApplicationContext(), "关闭脚本失败", Toast.LENGTH_SHORT).show();
                }
                break;
...
//监听TimePicker时间变化
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
        switch (compoundButton.getId()){
            ...
            case R.id.sw_cpdaily_forest:
                if (b){
                    if (b_accessblity) {
                        Toast.makeText(getApplicationContext(), "今日校园于" + hour + ":" + minute + "每日自动签到", Toast.LENGTH_SHORT).show();
    ...
    private void isCanBackground(){
        if (BackgroundAccess.canBackgroundStart(MainActivity.this)){
            ...
            Toast.makeText(getApplicationContext(), "请开启【后台弹出界面】权限", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void isOpenService(){
        ...
        if (IsServiceRunningUtil.isAccessibilitySettingsOn(this,"com.ycy.accessibilityservicetest.Service.MyAccessibilityService") == false){
            ...
            b_accessblity = false;
            Toast.makeText(getApplicationContext(), "未开启无障碍服务", Toast.LENGTH_SHORT).show();
